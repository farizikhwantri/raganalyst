...
# ---------------- Streamlit UI ----------------
st.set_page_config(page_title="RAG Analyst (OpenAI + Provider)", layout="wide")
st.title("RAG Analyst (OpenAI + Provider)")

st.sidebar.header("Index configuration")
index_prefix = st.sidebar.text_input("FAISS index prefix", OAI_INDEX_PREFIX)
metadata_path = st.sidebar.text_input("Metadata JSONL path", OAI_METADATA_PATH)
top_k = st.sidebar.slider("Top-K", 1, 10, 5, 1)

client = load_openai_client()

# Read manifest to match embedding dim/model
...
# Create provider
provider = OpenAIProvider(client=client, gen_model=OPENAI_MODEL, embed_model=OPENAI_EMBED_MODEL)
