...
# NEW: provider-based pipeline
from rag_module import HFProvider, rewrite_question, retrieve_with_expansion, generate_answer
...
@st.cache_resource
def load_generator():
    tokenizer = AutoTokenizer.from_pretrained(GEN_MODEL_NAME, token=HF_TOKEN)
...
# ---- Streamlit UI ----
st.set_page_config(page_title="Follow-up-aware RAG Analyst (HF Provider)", layout="wide")
st.title("Follow-up-aware RAG Analyst (HF + FAISS, Provider API)")
...
# Create provider
provider = HFProvider(tokenizer=tokenizer, gen_model=gen_model, embedder=embedder)
...
if "store" not in st.session_state or st.session_state.get("index_prefix") != index_prefix:
    try:
        ...
        st.session_state.store = load_index(index_prefix, emb_dim=emb_dim)
